...
*   **[06/07/2026]:** Variaveis de ambiente configuradas no projeto Vercel correto `kogna-online` para Production, Preview e Development. Deploy de producao realizado e validado em `https://kogna-online.vercel.app` e `/api/health`.
*   **[06/07/2026]:** Fluxo de onboarding refatorado: a home agora cria nova conexao de vendedor por nome e WhatsApp do gestor, gera link amigavel para `/vendedor/[id]`, cria a instancia Evolution e permite `whatsapp_vendedor` pendente ate o pareamento. A pagina do vendedor passa a carregar o QR Code automaticamente pela URL.
*   **[06/07/2026]:** Criada pagina segura `pages/admin/index.tsx` para gestor comercial, protegida por `ADMIN_SECRET` via query string. A tela exibe metricas simples, lista vendedores com status Evolution, permite adicionar vendedor por modal, copiar links de conexao e visualizar relatorios recentes em Markdown.
